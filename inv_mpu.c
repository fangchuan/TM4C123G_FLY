...
#if defined PART_LM4F232H5QD
#include "common.h"
#include "MPU6050.h"
#define i2c_write(a, b, c, d)   IIC_IO_MultipleWrite(a, b, d, c)
#define i2c_read(a, b, c, d)    IIC_IO_MultipleRead(a, b, d, c)
...
static int reg_int_cb(struct int_param_s *int_param)
{
    ...
    SysCtlPeripheralEnable(int_param->gpio_peripheral);
    GPIOPinTypeGPIOInput(int_param->gpio_port, int_param->gpio_pin);
	  GPIOPadConfigSet(int_param->gpio_port,int_param->gpio_pin,GPIO_STRENGTH_8MA,GPIO_PIN_TYPE_STD_WPU);
    GPIOIntTypeSet(int_param->gpio_port, int_param->gpio_pin, int_param->int_type);
    GPIOPinIntEnable(int_param->gpio_port, int_param->gpio_pin);
    //IntRegister(int_param->int_num, reg_int_handler);
    IntEnable(int_param->int_num);
    return 0;
}
...
const struct hw_s hw = {
    .addr           = 0x68,//mpu6050的IIC地址,若是MPU9150换成0X68
    .max_fifo       = 1024,
    .num_reg        = 128,
    .temp_sens      = 321,
    .temp_offset    = 0,
